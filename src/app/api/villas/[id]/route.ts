...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const villa = await db.villa.findUnique({ where: { id: params.id } });
    if (!villa) {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = await db.villa.findUnique({ where: { id: params.id } });
    if (!existing) {
      return NextResponse.json({ error: 'Villa non trouvée' }, { status: 404 });
    }
    ...
    const villa = await db.villa.update({
      where: { id: params.id },
      data,
    });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await db.villa.delete({ where: { id: params.id } });
